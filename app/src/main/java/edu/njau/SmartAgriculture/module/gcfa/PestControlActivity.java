...
import edu.njau.SmartAgriculture.module.gcfa.mvp.PestControlContract;
import edu.njau.SmartAgriculture.module.gcfa.mvp.PestControlPresenter;

/**
 * Created by zhaoxin5 on 2017/12/26.
 */

public class PestControlActivity extends CommonTopInfoActivity implements PestControlContract.View, View.OnClickListener {

    PestControlContract.Presenter mPresenter;

    List<BaseSpinnerAdapter.interf> mMonthList = new ArrayList<BaseSpinnerAdapter.interf>();
    @Bind(R.id.sp_gcfa_pc_time)
    Spinner mSpinnerMonth;
    BaseSpinnerAdapter mSpinnerAdapter;

    @Bind(R.id.rb_gcfa_pc_up)
    RadioButton mRadioUp;
    @Bind(R.id.rb_gcfa_pc_middle)
    RadioButton mRadioMiddle;
    @Bind(R.id.rb_gcfa_pc_down)
    RadioButton mRadioDown;
    @Bind(R.id.tv_gcfa_pc_leaf_age)
    TextView mLeafAge;
    @Bind(R.id.tv_gcfa_pc_stem_growth)
    TextView mStemGrowth;
    @Bind(R.id.tv_gcfa_pc_farming_operation)
    TextView mFarmingOperation;
    @Bind(R.id.tv_gcfa_pc_farming_operation_description)
    TextView mFarmingOperationDescription;
    @Bind(R.id.tv_gcfa_pc_pest_control)
    TextView mPestControl;
    @Bind(R.id.tv_gcfa_pc_pest_control_description)
    TextView mPestControlDescription;

    @Override
    protected int getSubViewId() {
        return R.layout.activity_gcfa_pest_control;
    }
    ...
    @Override
    public void setPresenter(PestControlContract.Presenter presenter) {
    ...
    public void onGetCommonTopInfoFromMonthAndMonthPeriod(AllInfo allInfo) {
        ...
        if (allInfo.getResponse().getHighplantlist().size() > 0) {
            setCurrentTypeValue(allInfo.getResponse().getHighplantlist().get(0).getPestcontrol().getPestcontrolname());
        } else {
    ...
    @OnClick({R.id.rb_gcfa_pc_up, R.id.rb_gcfa_pc_middle, R.id.rb_gcfa_pc_down})
    @Override
    public void onClick(View v) {
        if (v.getId() == R.id.rb_gcfa_pc_up ||
                v.getId() == R.id.rb_gcfa_pc_middle ||
                v.getId() == R.id.rb_gcfa_pc_down) {
            requestData();
        }
    }
